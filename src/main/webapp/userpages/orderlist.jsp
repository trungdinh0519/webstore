...
<html>
	<head>
		<link href="<c:url value="/styles/displayorder.css"/>"
	...
	<body>
		...
		<div>
			<div>
				<form id="orderListForm" action="order.do?method=search"
					method="post" name="plform1">
					...
					<input name="startDate" type="text" size="10" maxlength="30"
						readonly="true">
					<DIV id=testdiv1
						style="VISIBILITY:visible; POSITION:absolute; BACKGROUND-COLOR: white; layer-background-color: white; height:60px"></DIV>
					<a href=""
						onClick="cal1x.select(document.forms['orderListForm'].startDate,'anchor1x','dd/MM/yyyy'); return false;"
						NAME="anchor1x" ID="anchor1x"><img
							src="<c:url value="/images/b_calendar.png"/>" border="0"></img> </A>
					<fmt:message>label.toDate</fmt:message>
					<input name="endDate" type="text" size="10" maxlength="30"
						readonly="true">
					<a href=""
						onClick="cal1x.select(document.forms['orderListForm'].endDate,
			...
			<div id="order">
				<display:table name="orders" id="row" requestURI="/user/order.do"
					sort="external" partialList="true" pagesize="${pageSize}"
					size="resultSize" class="dh">
